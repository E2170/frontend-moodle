...
export default function Header() {
  ...
  return (
    <nav className="bg-[#19233e] text-white flex flex-col justify-center px-4 h-[66px] shadow-sm relative z-50 shrink-0">
      <div className="flex items-center justify-between w-full h-full">
        ...
        <div className="flex items-center gap-4 lg:gap-8 h-full">
          ...
          <img
            ...
            src={akuzemLogo}
            alt="AKUZEM"
            className="h-[40px] md:h-[50px] w-auto cursor-pointer p-0.5 object-contain"
          />
          ...
          <div className="hidden lg:flex gap-6 text-[15px] font-medium h-full items-center">
            {navItems.map((item) => (
              <button 
                ...
                className={`h-full flex items-center px-4 font-semibold transition-colors ${isActive(item.activePath) ? "bg-[#2d3246] text-white" : "text-[#9ca3af] hover:text-white"}`}
              >
                {item.label}
              </button>
            ))}
        ...
        <div className="flex items-center gap-3 md:gap-5 h-full">
          ...
          {/* Profile Dropdown */}
          <div 
            className="relative flex items-center gap-3 border-l border-white/20 pl-3 md:pl-5 h-full cursor-pointer hover:bg-white/5 px-2 transition-colors select-none"
          ...
          >
            {userInfo?.userpictureurl ? (
              ...
              <div className="w-8 h-8 md:w-9 md:h-9 bg-gray-600 rounded-full"></div>
            )}
            <div className="text-left hidden sm:block">
              <div className="text-[13px] font-semibold leading-tight text-white uppercase max-w-[120px] truncate">
            ...
            {activeDropdown === 'profile' && (
              <div 
                className="absolute top-[60px] right-2 bg-white text-[#495057] w-[260px] md:w-[280px] rounded-[14px] shadow-[0_5px_20px_rgba(0,0,0,0.15)] py-4 px-4 z-[100] border border-gray-100 cursor-default" 
                onClick={(e) => e.stopPropagation()}
              >
                {/* Triangle */}
                ...
                <div className="flex gap-2 mb-4">
                  <div className="flex-1 bg-[#0074b6] text-white rounded-[6px] p-2 flex items-center justify-center gap-2 cursor-pointer hover:bg-[#005a96] transition-colors">
                    ...
                    <div className="flex flex-col justify-center">
                      <span className="text-[8px] leading-tight text-white/80">Download on</span>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-[66px] left-0 right-0 bg-[#19233e] border-t border-white/10 shadow-lg z-40 max-h-[calc(100vh-66px)] overflow-y-auto">
          <div className="flex flex-col p-4 space-y-2">
            {navItems.map((item) => (
              <button 
                key={item.path}
                onClick={() => { navigate(item.path); setMobileMenuOpen(false); }} 
                className={`text-left w-full py-3 px-4 rounded-lg font-medium transition-colors ${isActive(item.activePath) ? "bg-[#2d3246] text-white" : "text-[#9ca3af] hover:bg-white/5 hover:text-white"}`}
              >
                {item.label}
              </button>
            ))}
