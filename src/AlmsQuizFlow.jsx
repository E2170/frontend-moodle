...
export function AlmsQuizActivityModal({ onClose, onSaveActivity }) {
  ...
  const [form, setForm] = useState({ name: "", intro: "", examNote: "", start: localISOTime, end: "", timelimit: "60", attempts: "1" });
          ...
          {activeTab === "AYARLAR" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
               ...
               <div className="grid grid-cols-2 gap-4">
                 <div>
                   <label className="block text-xs font-bold text-gray-700 mb-2">Sınav Süresi (Dakika)</label>
                   <input type="number" value={form.timelimit} onChange={e => setForm({...form, timelimit: e.target.value})} min="0" className="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm focus:border-blue-500 outline-none text-gray-700 shadow-sm transition-all focus:ring-2 ring-blue-100" />
                 </div>
                 <div>
                   <label className="block text-xs font-bold text-gray-700 mb-2">Giriş Hakkı (0 = Sınırsız)</label>
                   <select value={form.attempts} onChange={e => setForm({...form, attempts: e.target.value})} className="w-full border border-gray-300 rounded-md px-3 py-2.5 text-sm focus:border-blue-500 outline-none text-gray-700 shadow-sm transition-all focus:ring-2 ring-blue-100">
