...
export default function Calendar() {
  ...
  const getEventColor = (modulename) => {
    if (modulename === 'assign') return 'bg-[#4caf50]';
    if (modulename === 'quiz') return 'bg-[#9c27b0]';
    if (['bigbluebuttonbn', 'zoom', 'perculus'].includes(modulename)) return 'bg-[#2196f3]';
    if (modulename === 'forum') return 'bg-[#f44336]';
    return 'bg-[#ff9800]';
  };
                  ...
                  >
                    ...
                    {events
                      ...
                      .map((e, eIdx) => {
                        ...
                        const bgColor = getEventColor(e.modulename);
                        
                        return (
                          <div
                            key={eIdx}
                            className={`absolute left-1 right-1 p-2 rounded-[6px] text-white shadow-sm overflow-hidden z-10 ${bgColor}`}
